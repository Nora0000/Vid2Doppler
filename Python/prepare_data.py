...
def segment_align(path, path_of_segmentation, fps=180):
	...
	for seg in segs:
		...
		start_ind, stop_ind = align_index(t1, start_new, fps), align_index(t2, start_new, fps)
...
def load_data(path, filename, path_seg, filter=False, filename_seg="segmentation.npy", label=0, path_of_segmentation_rely=None, seg_length=50):
	...
	for seg in segs:
		...
		# sns.heatmap(d)
		# plt.show()
		d_seg.append(interpolate(d, width=seg_length))
...
if __name__ == "__main__":
	...
	path = "/home/mengjingliu/Vid2Doppler/data/2023_07_19/HAR4"
	...
	for ac in act:
		...
		# 	X = np.vstack((X, data))
		# 	Y = np.concatenate((Y, label))
	np.save(os.path.join(path, "X_2.npy"), X)
	np.save(os.path.join(path, "Y_2.npy"), Y)
